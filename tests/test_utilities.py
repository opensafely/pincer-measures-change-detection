    ...
    return pd.DataFrame(
        {
            ...
            "earliest_medications_x_month_3": pd.to_datetime(
                pd.Series(["2019-10-01", np.nan, np.nan, "2019-10-10", np.nan])
            ),
            "earliest_medications_x_month_2": pd.to_datetime(
                pd.Series([np.nan, np.nan, np.nan, np.nan, "2019-11-10"])
            ),
            "earliest_medications_x_month_1": pd.to_datetime(
                pd.Series(["2019-12-20", "2019-12-25", np.nan, np.nan, np.nan])
            ),
            "latest_medications_x_month_3": pd.to_datetime(
                pd.Series(["2019-10-01", np.nan, np.nan, "2019-10-10", np.nan])
            ),
            "latest_medications_x_month_2": pd.to_datetime(
                pd.Series([np.nan, np.nan, np.nan, np.nan, "2019-11-10"])
            ),
            "latest_medications_x_month_1": pd.to_datetime(
                pd.Series(["2019-12-20", "2019-12-25", np.nan, np.nan, np.nan])
            ),
            "earliest_medications_y_month_3": pd.to_datetime(
                pd.Series([np.nan, "2019-10-01", np.nan, "2019-10-10", np.nan])
            ),
            "earliest_medications_y_month_2": pd.to_datetime(
                pd.Series(["2019-11-25", np.nan, np.nan, np.nan, np.nan])
            ),
            "earliest_medications_y_month_1": pd.to_datetime(
                pd.Series([np.nan, np.nan, np.nan, np.nan, np.nan])
            ),
            "latest_medications_y_month_3": pd.to_datetime(
                pd.Series([np.nan, "2019-10-01", np.nan, "2019-10-10", np.nan])
            ),
            "latest_medications_y_month_2": pd.to_datetime(
                pd.Series(["2019-11-25", np.nan, np.nan, np.nan, np.nan])
            ),
            "latest_medications_y_month_1": pd.to_datetime(
                pd.Series([np.nan, np.nan, np.nan, np.nan, np.nan])
            ),
            "msoa": pd.Series(
...
def multiple_indicator_list():
    ...
    return ["variable_a", "variable_b", "variable_d"]
...
class TestMatchInputFiles:
    ...
    def test_bad_file_format(self, filename):
        bad_file_format = filename(
            name="input_01-01-2020.feather"
...
def test_get_date_input_file(filename):
    ...
    try:
        ...
        assert (
            True
        ), f"Badly formatted file {bad_file_format} does throw an exception {exc}"
...
def test_validate_directory():
    ...
    try:
        ...
        assert (
            False
        ), f"Newly created temporary directory {str(temp_dir.name)} does not throw an exception {exc}"
    ...
    try:
        ...
        assert (
            True
        ), f"Newly deleted temporary directory {str(temp_dir.name)} does throw an exception {exc}"
...
def test_join_ethnicity_region(tmp_path, input_file, input_file_ethnicity):
    ...
    with patch.object(utilities, "OUTPUT_DIR", tmp_path):
        ...
        input_file_ethnicity.to_feather(
            utilities.OUTPUT_DIR / "input_ethnicity.feather"
        )
        ...
        testing.assert_series_equal(
            merged_df["ethnicity"], pd.Series([1, 2, 2, 1, 3], name="ethnicity")
        )
...
def test_redact_small_numbers(counts_table):
    ...
    counts_table["rate"] = utilities.calculate_rate(
        counts_table, "numerator", "denominator", 1
    )

    counts_table_redacted = utilities.redact_small_numbers(
        counts_table, 5, "numerator", "denominator", "rate", "date"
    )
...
def test_calculate_rate():
    ...
    mt["num_per_thousand"] = utilities.calculate_rate(
        mt, "systolic_bp", "population", 1000
    )
...
def test_compute_redact_deciles(measure_table_for_deciles):
    ...
    obs = utilities.compute_redact_deciles(
        measure_table_for_deciles, "date", "count", "rate"
    )
...
def test_group_low_values(composite_indicator_table):
    ...
    obs = utilities.group_low_values(
        composite_indicator_table, "count", "denominator", "num_indicators"
    )
